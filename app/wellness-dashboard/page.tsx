// // // // // "use client"

// // // // // import { useEffect, useState } from "react"
// // // // // import { useRouter } from "next/navigation"
// // // // // import { AnimatedBackground } from "@/components/animated-background"
// // // // // import { GradientText } from "@/components/gradient-text"
// // // // // import { ScrollReveal } from "@/components/scroll-reveal"
// // // // // import { ContentModal } from "@/components/content-modal"
// // // // // import { doshaData } from "@/lib/dosha-data"
// // // // // import FloatingNavbar from "@/components/floating-navbar"
// // // // // import YogaPosesModal from "@/components/yoga-poses-modal"
// // // // // import { FloatingElements3D } from "@/components/3d-floating-elements"
// // // // // import { Lotus3D } from "@/components/3d-lotus"
// // // // // import { Chakra3D } from "@/components/3d-chakra"
// // // // // import { Footer } from "@/components/footer"
// // // // // import CustomCursor from "@/components/CustomCursor"
// // // // // import { AuroraBackground } from "@/components/aurora-background"

// // // // // export default function WellnessDashboardPage() {
// // // // //   const [hover, setHover] = useState(false);
// // // // //   const [doshaType, setDoshaType] = useState<string>("")
// // // // //   const [doshaScores, setDoshaScores] = useState<Record<string, number>>({})
// // // // //   const [modalContent, setModalContent] = useState<{
// // // // //     isOpen: boolean
// // // // //     title: string
// // // // //     items: string[]
// // // // //     icon: string
// // // // //     gradient: string
// // // // //   }>({
// // // // //     isOpen: false,
// // // // //     title: "",
// // // // //     items: [],
// // // // //     icon: "",
// // // // //     gradient: "",
// // // // //   })
// // // // //   const [isYogaPosesOpen, setIsYogaPosesOpen] = useState(false)
// // // // //   const router = useRouter()

// // // // //   useEffect(() => {
// // // // //     const result = localStorage.getItem("doshaResult")
// // // // //     const scores = localStorage.getItem("doshaScores")

// // // // //     if (result && scores) {
// // // // //       setDoshaType(result)
// // // // //       setDoshaScores(JSON.parse(scores))
// // // // //     } else {
// // // // //       router.push("/dosha-quiz")
// // // // //     }
// // // // //   }, [router])

// // // // //   const currentDosha = doshaData[doshaType]

// // // // //   if (!currentDosha) return null

// // // // //   const handleRetakeQuiz = () => {
// // // // //     localStorage.removeItem("doshaResult")
// // // // //     localStorage.removeItem("doshaScores")
// // // // //     router.push("/dosha-quiz")
// // // // //   }

// // // // //   const openModal = (title: string, items: string[], icon: string, gradient: string) => {
// // // // //     setModalContent({
// // // // //       isOpen: true,
// // // // //       title,
// // // // //       items,
// // // // //       icon,
// // // // //       gradient,
// // // // //     })
// // // // //   }

// // // // //   const closeModal = () => {
// // // // //     setModalContent((prev) => ({ ...prev, isOpen: false }))
// // // // //   }

// // // // //   const dashboardSections = [
// // // // //     {
// // // // //       key: "diet",
// // // // //       title: currentDosha.diet.title,
// // // // //       items: currentDosha.diet.items,
// // // // //       icon: "🥗",
// // // // //       gradient: currentDosha.colors.primary,
// // // // //     },
// // // // //     {
// // // // //       key: "lifestyle",
// // // // //       title: currentDosha.lifestyle.title,
// // // // //       items: currentDosha.lifestyle.items,
// // // // //       icon: "🏠",
// // // // //       gradient: currentDosha.colors.primary,
// // // // //     },
// // // // //     {
// // // // //       key: "yoga",
// // // // //       title: currentDosha.yoga.title,
// // // // //       items: currentDosha.yoga.items,
// // // // //       icon: "🧘‍♀️",
// // // // //       gradient: currentDosha.colors.primary,
// // // // //     },
// // // // //     {
// // // // //       key: "remedies",
// // // // //       title: currentDosha.remedies.title,
// // // // //       items: currentDosha.remedies.items,
// // // // //       icon: "🌿",
// // // // //       gradient: currentDosha.colors.primary,
// // // // //     },
// // // // //   ]

// // // // //   return (
// // // // //     <div className="min-h-screen bg-gradient-to-br from-slate-900 via-purple-900 to-slate-900">
// // // // //       <FloatingNavbar />
// // // // //       {/* <AnimatedBackground /> */}
// // // // //       <AuroraBackground/>
// // // // //       <FloatingElements3D />
// // // // //       <CustomCursor isHover={hover}/>
      

// // // // //       <div className="relative z-10">
// // // // //         {/* Header */}
// // // // //         <div className="pt-20 pb-12 px-4">
// // // // //           <div className="max-w-6xl mx-auto text-center">
// // // // //             <ScrollReveal>
// // // // //               <GradientText className="text-4xl md:text-6xl font-serif font-bold mb-4">
// // // // //                 Your Wellness Dashboard
// // // // //               </GradientText>
// // // // //               <p className="text-xl text-slate-300 mb-8">
// // // // //                 Personalized recommendations for your {currentDosha.name} constitution
// // // // //               </p>
// // // // //             </ScrollReveal>

// // // // //             <ScrollReveal delay={200}>
// // // // //               <div className="flex justify-center items-center space-x-8 mb-8">
// // // // //                 <div className="text-center">
// // // // //                   <div className="text-2xl font-bold text-blue-400">{doshaScores.vata || 0}</div>
// // // // //                   <div className="text-sm text-slate-400">Vata</div>
// // // // //                 </div>
// // // // //                 <div className="text-center">
// // // // //                   <div className="text-2xl font-bold text-orange-400">{doshaScores.pitta || 0}</div>
// // // // //                   <div className="text-sm text-slate-400">Pitta</div>
// // // // //                 </div>
// // // // //                 <div className="text-center">
// // // // //                   <div className="text-2xl font-bold text-green-400">{doshaScores.kapha || 0}</div>
// // // // //                   <div className="text-sm text-slate-400">Kapha</div>
// // // // //                 </div>
// // // // //               </div>
// // // // //             </ScrollReveal>

// // // // //             <ScrollReveal delay={400}>
// // // // //               <div className="flex flex-wrap justify-center gap-4">
// // // // //                 <button
// // // // //                   onClick={handleRetakeQuiz}
// // // // //                   className="px-6 py-3 glass-card rounded-xl text-slate-200 hover:bg-white/20 transition-all duration-300 transform hover:scale-105"
// // // // //                 >
// // // // //                   Re-evaluate My Dosha
// // // // //                 </button>
// // // // //                 <button
// // // // //                   onClick={() => setIsYogaPosesOpen(true)}
// // // // //                   className="px-6 py-3 glass-card rounded-xl text-slate-200 hover:bg-white/20 transition-all duration-300 transform hover:scale-105 flex items-center space-x-2"
// // // // //                 >
// // // // //                   <span>🧘‍♀️</span>
// // // // //                   <span>Explore Yoga Poses</span>
// // // // //                 </button>
// // // // //               </div>
// // // // //             </ScrollReveal>
// // // // //           </div>
// // // // //         </div>

// // // // //         {/* Recommendations Grid */}
// // // // //         <div className="max-w-6xl mx-auto px-4 pb-20">
// // // // //           <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-8">
// // // // //             {dashboardSections.map((section, index) => (
// // // // //               <ScrollReveal key={section.key} delay={index * 100} animation="slide-up">
// // // // //                 <div className="group relative">
// // // // //                   <button
// // // // //                     onClick={() => openModal(section.title, section.items, section.icon, section.gradient)}
// // // // //                     className="w-full text-left glass-card rounded-3xl p-8 hover:bg-white/10 transition-all duration-500 transform hover:scale-105 hover:-translate-y-2"
// // // // //                   >
// // // // //                     {/* Icon */}
// // // // //                     <div className="mb-6">
// // // // //                       <div
// // // // //                         className={`w-16 h-16 rounded-2xl bg-gradient-to-r ${section.gradient} flex items-center justify-center text-2xl mb-4 animate-pulse`}
// // // // //                       >
// // // // //                         {section.icon}
// // // // //                       </div>
// // // // //                       <h3 className="text-2xl font-serif font-bold text-slate-200 mb-2">{section.title}</h3>
// // // // //                     </div>

// // // // //                     {/* Preview of first 2 items */}
// // // // //                     <div className="space-y-3">
// // // // //                       {section.items.slice(0, 2).map((item, itemIndex) => (
// // // // //                         <div
// // // // //                           key={itemIndex}
// // // // //                           className="flex items-start space-x-3 group-hover:translate-x-1 transition-transform duration-300"
// // // // //                           style={{ transitionDelay: `${itemIndex * 50}ms` }}
// // // // //                         >
// // // // //                           <div
// // // // //                             className={`w-2 h-2 rounded-full bg-gradient-to-r ${section.gradient} mt-2 flex-shrink-0`}
// // // // //                           />
// // // // //                           <p className="text-slate-300 text-sm leading-relaxed">{item}</p>
// // // // //                         </div>
// // // // //                       ))}
// // // // //                       {section.items.length > 2 && (
// // // // //                         <div className="text-center pt-4">
// // // // //                           <span className="text-slate-400 text-sm">Click to see all recommendations →</span>
// // // // //                         </div>
// // // // //                       )}
// // // // //                     </div>

// // // // //                     {/* Hover Glow Effect */}
// // // // //                     <div
// // // // //                       className={`absolute inset-0 rounded-3xl bg-gradient-to-r ${section.gradient} opacity-0 group-hover:opacity-10 transition-opacity duration-500 pointer-events-none`}
// // // // //                     />
// // // // //                   </button>
// // // // //                 </div>
// // // // //               </ScrollReveal>
// // // // //             ))}
// // // // //           </div>
// // // // //         </div>

// // // // //         {/* Daily Practice Section */}
// // // // //         <ScrollReveal>
// // // // //           <div className="max-w-4xl mx-auto px-4 pb-20">
// // // // //             <div className="glass-card rounded-3xl p-8">
// // // // //               <div className="text-center mb-8">
// // // // //                 <GradientText className="text-3xl font-serif font-bold mb-4">Daily Practice Reminder</GradientText>
// // // // //                 <p className="text-slate-300">Consistency is key to balancing your {currentDosha.name} constitution</p>
// // // // //               </div>

// // // // //               <div className="grid md:grid-cols-3 gap-6">
// // // // //                 <div className="text-center">
// // // // //                   <div className="w-12 h-12 rounded-full bg-gradient-to-r from-teal-400 to-purple-400 flex items-center justify-center text-xl mb-3 mx-auto animate-spin-slow">
// // // // //                     🧘
// // // // //                   </div>
// // // // //                   <h4 className="font-semibold text-slate-200 mb-2">Morning Practice</h4>
// // // // //                   <p className="text-sm text-slate-400">Start with meditation and pranayama</p>
// // // // //                 </div>

// // // // //                 <div className="text-center">
// // // // //                   <div className="w-12 h-12 rounded-full bg-gradient-to-r from-purple-400 to-pink-400 flex items-center justify-center text-xl mb-3 mx-auto animate-bounce-slow">
// // // // //                     🍃
// // // // //                   </div>
// // // // //                   <h4 className="font-semibold text-slate-200 mb-2">Mindful Eating</h4>
// // // // //                   <p className="text-sm text-slate-400">Follow your dosha-specific diet</p>
// // // // //                 </div>

// // // // //                 <div className="text-center">
// // // // //                   <div className="w-12 h-12 rounded-full bg-gradient-to-r from-pink-400 to-orange-400 flex items-center justify-center text-xl mb-3 mx-auto animate-pulse">
// // // // //                     🌙
// // // // //                   </div>
// // // // //                   <h4 className="font-semibold text-slate-200 mb-2">Evening Routine</h4>
// // // // //                   <p className="text-sm text-slate-400">Wind down with gentle yoga</p>
// // // // //                 </div>
// // // // //               </div>
// // // // //             </div>
// // // // //           </div>
// // // // //         </ScrollReveal>
// // // // //       </div>

// // // // //       <ContentModal
// // // // //         isOpen={modalContent.isOpen}
// // // // //         onClose={closeModal}
// // // // //         title={modalContent.title}
// // // // //         items={modalContent.items}
// // // // //         icon={modalContent.icon}
// // // // //         gradient={modalContent.gradient}
// // // // //       />

// // // // //       <YogaPosesModal isOpen={isYogaPosesOpen} onClose={() => setIsYogaPosesOpen(false)} />

// // // // //       <div className="absolute top-32 left-10 opacity-30">
// // // // //         <Lotus3D />
// // // // //       </div>
// // // // //       <div className="absolute top-40 right-20 opacity-40">
// // // // //         <Chakra3D size={80} color="teal" speed={0.5} />
// // // // //       </div>
// // // // //       <div className="absolute bottom-40 left-1/4 opacity-25">
// // // // //         <Chakra3D size={60} color="purple" speed={1.2} />
// // // // //       </div>

// // // // //       <style jsx>{`
// // // // //         @keyframes spin-slow {
// // // // //           from { transform: rotate(0deg); }
// // // // //           to { transform: rotate(360deg); }
// // // // //         }

// // // // //         @keyframes bounce-slow {
// // // // //           0%, 100% { transform: translateY(0); }
// // // // //           50% { transform: translateY(-10px); }
// // // // //         }

// // // // //         .animate-spin-slow {
// // // // //           animation: spin-slow 8s linear infinite;
// // // // //         }

// // // // //         .animate-bounce-slow {
// // // // //           animation: bounce-slow 3s ease-in-out infinite;
// // // // //         }

// // // // //         .glass-card {
// // // // //           background: rgba(255, 255, 255, 0.1);
// // // // //           backdrop-filter: blur(10px);
// // // // //           border: 1px solid rgba(255, 255, 255, 0.2);
// // // // //         }
// // // // //       `}</style>
// // // // //       <Footer/>
// // // // //     </div>
// // // // //   )
// // // // // }
// // // // "use client"

// // // // import { useEffect, useState } from "react"
// // // // import { useRouter } from "next/navigation"
// // // // import { AnimatedBackground } from "@/components/animated-background"
// // // // import { GradientText } from "@/components/gradient-text"
// // // // import { ScrollReveal } from "@/components/scroll-reveal"
// // // // import { ContentModal } from "@/components/content-modal"
// // // // import { doshaData } from "@/lib/dosha-data"
// // // // import FloatingNavbar from "@/components/floating-navbar"
// // // // import YogaPosesModal from "@/components/yoga-poses-modal"
// // // // import { FloatingElements3D } from "@/components/3d-floating-elements"
// // // // import { Lotus3D } from "@/components/3d-lotus"
// // // // import { Chakra3D } from "@/components/3d-chakra"
// // // // import { Footer } from "@/components/footer"
// // // // import CustomCursor from "@/components/CustomCursor"
// // // // import { AuroraBackground } from "@/components/aurora-background"

// // // // // shadcn imports
// // // // import { Card, CardContent, CardHeader, CardTitle } from "@/components/ui/card"
// // // // import { Progress } from "@/components/ui/progress"
// // // // import { Badge } from "@/components/ui/badge"
// // // // // import { Progress } from "@/components/ui/progress"
// // // // // import { Badge } from "@/components/ui/badge"
// // // // import BreathingExercise from "@/components/BreathingExercise";
// // // // import Chatbot from "@/components/Chatbot"


// // // // export default function WellnessDashboardPage() {
// // // //   const [hover, setHover] = useState(false);
// // // //   const [doshaType, setDoshaType] = useState<string>("")
// // // //   const [doshaScores, setDoshaScores] = useState<Record<string, number>>({})
// // // //   const [modalContent, setModalContent] = useState<{
// // // //     isOpen: boolean
// // // //     title: string
// // // //     items: string[]
// // // //     icon: string
// // // //     gradient: string
// // // //   }>({
// // // //     isOpen: false,
// // // //     title: "",
// // // //     items: [],
// // // //     icon: "",
// // // //     gradient: "",
// // // //   })
// // // //   const [isYogaPosesOpen, setIsYogaPosesOpen] = useState(false)
// // // //   const router = useRouter()

// // // //   useEffect(() => {
// // // //     const result = localStorage.getItem("doshaResult")
// // // //     const scores = localStorage.getItem("doshaScores")

// // // //     if (result && scores) {
// // // //       setDoshaType(result)
// // // //       setDoshaScores(JSON.parse(scores))
// // // //     } else {
// // // //       router.push("/dosha-quiz")
// // // //     }
// // // //   }, [router])

// // // //   const currentDosha = doshaData[doshaType]

// // // //   if (!currentDosha) return null

// // // //   const handleRetakeQuiz = () => {
// // // //     localStorage.removeItem("doshaResult")
// // // //     localStorage.removeItem("doshaScores")
// // // //     router.push("/dosha-quiz")
// // // //   }

// // // //   const openModal = (title: string, items: string[], icon: string, gradient: string) => {
// // // //     setModalContent({
// // // //       isOpen: true,
// // // //       title,
// // // //       items,
// // // //       icon,
// // // //       gradient,
// // // //     })
// // // //   }

// // // //   const closeModal = () => {
// // // //     setModalContent((prev) => ({ ...prev, isOpen: false }))
// // // //   }

// // // //   const dashboardSections = [
// // // //     {
// // // //       key: "diet",
// // // //       title: currentDosha.diet.title,
// // // //       items: currentDosha.diet.items,
// // // //       icon: "🥗",
// // // //       gradient: currentDosha.colors.primary,
// // // //     },
// // // //     {
// // // //       key: "lifestyle",
// // // //       title: currentDosha.lifestyle.title,
// // // //       items: currentDosha.lifestyle.items,
// // // //       icon: "🏠",
// // // //       gradient: currentDosha.colors.primary,
// // // //     },
// // // //     {
// // // //       key: "yoga",
// // // //       title: currentDosha.yoga.title,
// // // //       items: currentDosha.yoga.items,
// // // //       icon: "🧘‍♀️",
// // // //       gradient: currentDosha.colors.primary,
// // // //     },
// // // //     {
// // // //       key: "remedies",
// // // //       title: currentDosha.remedies.title,
// // // //       items: currentDosha.remedies.items,
// // // //       icon: "🌿",
// // // //       gradient: currentDosha.colors.primary,
// // // //     },
// // // //   ]

// // // //   return (
// // // //     <div className="min-h-screen bg-gradient-to-br from-slate-900 via-purple-900 to-slate-900">
// // // //       <FloatingNavbar />
// // // //       <AuroraBackground/>
// // // //       <FloatingElements3D />
// // // //       <CustomCursor isHover={hover}/>

// // // //       <div className="relative z-10">
// // // //         {/* Header */}
// // // //         <div className="pt-20 pb-12 px-4">
// // // //           <div className="max-w-6xl mx-auto text-center">
// // // //             <ScrollReveal>
// // // //               <GradientText className="text-4xl md:text-6xl font-serif font-bold mb-4">
// // // //                 Your Wellness Dashboard
// // // //               </GradientText>
// // // //               <p className="text-xl text-slate-300 mb-8">
// // // //                 Personalized recommendations for your {currentDosha.name} constitution
// // // //               </p>
// // // //             </ScrollReveal>

// // // //             <ScrollReveal delay={200}>
// // // //               <div className="flex justify-center items-center space-x-8 mb-8">
// // // //                 <div className="text-center">
// // // //                   <div className="text-2xl font-bold text-blue-400">{doshaScores.vata || 0}</div>
// // // //                   <div className="text-sm text-slate-400">Vata</div>
// // // //                 </div>
// // // //                 <div className="text-center">
// // // //                   <div className="text-2xl font-bold text-orange-400">{doshaScores.pitta || 0}</div>
// // // //                   <div className="text-sm text-slate-400">Pitta</div>
// // // //                 </div>
// // // //                 <div className="text-center">
// // // //                   <div className="text-2xl font-bold text-green-400">{doshaScores.kapha || 0}</div>
// // // //                   <div className="text-sm text-slate-400">Kapha</div>
// // // //                 </div>
// // // //               </div>
// // // //             </ScrollReveal>

// // // //             <ScrollReveal delay={400}>
// // // //               <div className="flex flex-wrap justify-center gap-4">
// // // //                 <button
// // // //                   onClick={handleRetakeQuiz}
// // // //                   className="px-6 py-3 glass-card rounded-xl text-slate-200 hover:bg-white/20 transition-all duration-300 transform hover:scale-105"
// // // //                 >
// // // //                   Re-evaluate My Dosha
// // // //                 </button>
// // // //                 <button
// // // //                   onClick={() => setIsYogaPosesOpen(true)}
// // // //                   className="px-6 py-3 glass-card rounded-xl text-slate-200 hover:bg-white/20 transition-all duration-300 transform hover:scale-105 flex items-center space-x-2"
// // // //                 >
// // // //                   <span>🧘‍♀️</span>
// // // //                   <span>Explore Yoga Poses</span>
// // // //                 </button>
// // // //               </div>
// // // //             </ScrollReveal>
// // // //           </div>
// // // //         </div>

// // // //       {/* Daily Tracker + Streak Section */}
// // // //         <ScrollReveal>
// // // //           <div className="max-w-6xl mx-auto px-4 pb-20 grid md:grid-cols-2 gap-8">
// // // //             {/* Daily Tracker */}
// // // //             <Card className="glass-card rounded-3xl">
// // // //               <CardHeader>
// // // //                 <CardTitle className="text-slate-200 font-serif text-xl">Daily Tracker</CardTitle>
// // // //               </CardHeader>
// // // //               <CardContent className="space-y-4">
// // // //                 <div>
// // // //                   <p className="text-slate-300 text-sm mb-2">Morning Meditation</p>
// // // //                   <Progress value={30} className="h-2" />
// // // //                 </div>
// // // //                 <div>
// // // //                   <p className="text-slate-300 text-sm mb-2">Yoga Practice</p>
// // // //                   <Progress value={45} className="h-2" />
// // // //                 </div>
// // // //                 <div>
// // // //                   <p className="text-slate-300 text-sm mb-2">Mindful Eating</p>
// // // //                   <Progress value={15 } className="h-2" />
// // // //                 </div>
// // // //               </CardContent>
// // // //             </Card>

// // // //             {/* Streak Section */}
// // // //             <Card className="glass-card rounded-3xl">
// // // //               <CardHeader>
// // // //                 <CardTitle className="text-slate-200 font-serif text-xl">Your Streak</CardTitle>
// // // //               </CardHeader>
// // // //               <CardContent className="flex flex-col items-center space-y-4">
// // // //                 <div className="text-6xl font-bold text-teal-400">🔥</div>
// // // //                 <p className="text-slate-400">Start your Streak</p>
// // // //                 <div className="flex gap-2">
// // // //                   {[...Array(7)].map((_, i) => (
// // // //                     <Badge key={i} className={`rounded-full px-3 py-1 ${i < 7 ? "bg-gradient-to-r from-teal-400 to-purple-400 text-white" : "bg-slate-600"}`}>
// // // //                       {i + 1}
// // // //                     </Badge>
// // // //                   ))}
// // // //                 </div>
// // // //               </CardContent>
// // // //             </Card>
// // // //           </div>
// // // //         </ScrollReveal>
// // // //         {/* Breathing Exercise Section */}
// // // // {/* <ScrollReveal>
// // // //   <div className="max-w-6xl mx-auto px-4 pb-20">
// // // //     <Card className="glass-card rounded-3xl p-6">
// // // //       <CardHeader>
// // // //         <CardTitle className="text-slate-200 font-serif text-xl">
// // // //           Breathing Exercise
// // // //         </CardTitle>
// // // //       </CardHeader>
// // // //       <CardContent>
// // // //         <BreathingExercise />
// // // //       </CardContent>
// // // //     </Card>
// // // //   </div>
// // // // </ScrollReveal> */}
// // // // <ScrollReveal>
// // // //   <div className="flex justify-center items-center pb-20">
// // // //     <Card className="glass-card rounded-3xl p-6 w-[22rem]">
// // // //       <CardHeader>
// // // //         <CardTitle className="text-slate-200 font-serif text-xl text-center">
// // // //           Breathing Exercise
// // // //         </CardTitle>
// // // //       </CardHeader>
// // // //       <CardContent>
// // // //         <BreathingExercise />
// // // //       </CardContent>
// // // //     </Card>
// // // //   </div>
// // // // </ScrollReveal>











// // // //         {/* Recommendations Grid */}
// // // //         <div className="max-w-6xl mx-auto px-4 pb-20">
// // // //           <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-8">
// // // //             {dashboardSections.map((section, index) => (
// // // //               <ScrollReveal key={section.key} delay={index * 100} animation="slide-up">
// // // //                 <div className="group relative">
// // // //                   <button
// // // //                     onClick={() => openModal(section.title, section.items, section.icon, section.gradient)}
// // // //                     className="w-full text-left glass-card rounded-3xl p-8 hover:bg-white/10 transition-all duration-500 transform hover:scale-105 hover:-translate-y-2"
// // // //                   >
// // // //                     {/* Icon */}
// // // //                     <div className="mb-6">
// // // //                       <div
// // // //                         className={`w-16 h-16 rounded-2xl bg-gradient-to-r ${section.gradient} flex items-center justify-center text-2xl mb-4 animate-pulse`}
// // // //                       >
// // // //                         {section.icon}
// // // //                       </div>
// // // //                       <h3 className="text-2xl font-serif font-bold text-slate-200 mb-2">{section.title}</h3>
// // // //                     </div>

// // // //                     {/* Preview of first 2 items */}
// // // //                     <div className="space-y-3">
// // // //                       {section.items.slice(0, 2).map((item, itemIndex) => (
// // // //                         <div
// // // //                           key={itemIndex}
// // // //                           className="flex items-start space-x-3 group-hover:translate-x-1 transition-transform duration-300"
// // // //                           style={{ transitionDelay: `${itemIndex * 50}ms` }}
// // // //                         >
// // // //                           <div
// // // //                             className={`w-2 h-2 rounded-full bg-gradient-to-r ${section.gradient} mt-2 flex-shrink-0`}
// // // //                           />
// // // //                           <p className="text-slate-300 text-sm leading-relaxed">{item}</p>
// // // //                         </div>
// // // //                       ))}
// // // //                       {section.items.length > 2 && (
// // // //                         <div className="text-center pt-4">
// // // //                           <span className="text-slate-400 text-sm">Click to see all recommendations →</span>
// // // //                         </div>
// // // //                       )}
// // // //                     </div>

// // // //                     {/* Hover Glow Effect */}
// // // //                     <div
// // // //                       className={`absolute inset-0 rounded-3xl bg-gradient-to-r ${section.gradient} opacity-0 group-hover:opacity-10 transition-opacity duration-500 pointer-events-none`}
// // // //                     />
// // // //                   </button>
// // // //                 </div>
// // // //               </ScrollReveal>
// // // //             ))}
// // // //           </div>
// // // //         </div>

// // // //         {/* Daily Practice Section */}
// // // //         <ScrollReveal>
// // // //           <div className="max-w-4xl mx-auto px-4 pb-20">
// // // //             <div className="glass-card rounded-3xl p-8">
// // // //               <div className="text-center mb-8">
// // // //                 <GradientText className="text-3xl font-serif font-bold mb-4">Daily Practice Reminder</GradientText>
// // // //                 <p className="text-slate-300">Consistency is key to balancing your {currentDosha.name} constitution</p>
// // // //               </div>

// // // //               <div className="grid md:grid-cols-3 gap-6">
// // // //                 <div className="text-center">
// // // //                   <div className="w-12 h-12 rounded-full bg-gradient-to-r from-teal-400 to-purple-400 flex items-center justify-center text-xl mb-3 mx-auto animate-spin-slow">
// // // //                     🧘
// // // //                   </div>
// // // //                   <h4 className="font-semibold text-slate-200 mb-2">Morning Practice</h4>
// // // //                   <p className="text-sm text-slate-400">Start with meditation and pranayama</p>
// // // //                 </div>

// // // //                 <div className="text-center">
// // // //                   <div className="w-12 h-12 rounded-full bg-gradient-to-r from-purple-400 to-pink-400 flex items-center justify-center text-xl mb-3 mx-auto animate-bounce-slow">
// // // //                     🍃
// // // //                   </div>
// // // //                   <h4 className="font-semibold text-slate-200 mb-2">Mindful Eating</h4>
// // // //                   <p className="text-sm text-slate-400">Follow your dosha-specific diet</p>
// // // //                 </div>

// // // //                 <div className="text-center">
// // // //                   <div className="w-12 h-12 rounded-full bg-gradient-to-r from-pink-400 to-orange-400 flex items-center justify-center text-xl mb-3 mx-auto animate-pulse">
// // // //                     🌙
// // // //                   </div>
// // // //                   <h4 className="font-semibold text-slate-200 mb-2">Evening Routine</h4>
// // // //                   <p className="text-sm text-slate-400">Wind down with gentle yoga</p>
// // // //                 </div>
// // // //               </div>
// // // //             </div>
// // // //           </div>
// // // //         </ScrollReveal>

        
// // // //       </div>

// // // //       <ContentModal
// // // //         isOpen={modalContent.isOpen}
// // // //         onClose={closeModal}
// // // //         title={modalContent.title}
// // // //         items={modalContent.items}
// // // //         icon={modalContent.icon}
// // // //         gradient={modalContent.gradient}
// // // //       />

// // // //       <YogaPosesModal isOpen={isYogaPosesOpen} onClose={() => setIsYogaPosesOpen(false)} />

// // // //       <div className="absolute top-32 left-10 opacity-30">
// // // //         <Lotus3D />
// // // //       </div>
// // // //       <div className="absolute top-40 right-20 opacity-40">
// // // //         <Chakra3D size={80} color="teal" speed={0.5} />
// // // //       </div>
// // // //       <div className="absolute bottom-40 left-1/4 opacity-25">
// // // //         <Chakra3D size={60} color="purple" speed={1.2} />
// // // //       </div>

// // // //       <style jsx>{`
// // // //         @keyframes spin-slow {
// // // //           from { transform: rotate(0deg); }
// // // //           to { transform: rotate(360deg); }
// // // //         }

// // // //         @keyframes bounce-slow {
// // // //           0%, 100% { transform: translateY(0); }
// // // //           50% { transform: translateY(-10px); }
// // // //         }

// // // //         .animate-spin-slow {
// // // //           animation: spin-slow 8s linear infinite;
// // // //         }

// // // //         .animate-bounce-slow {
// // // //           animation: bounce-slow 3s ease-in-out infinite;
// // // //         }

// // // //         .glass-card {
// // // //           background: rgba(255, 255, 255, 0.1);
// // // //           backdrop-filter: blur(10px);
// // // //           border: 1px solid rgba(255, 255, 255, 0.2);
// // // //         }
// // // //       `}</style>
// // // //       {/* Chatbot fixed at bottom-right */}
// // // //             <Chatbot />
// // // //       <Footer/>
// // // //     </div>
// // // //   )
// // // // }
// "use client"

// import { useEffect, useState } from "react"
// import { useRouter } from "next/navigation"
// import { AnimatedBackground } from "@/components/animated-background"
// import { GradientText } from "@/components/gradient-text"
// import { ScrollReveal } from "@/components/scroll-reveal"
// import { ContentModal } from "@/components/content-modal"
// import { doshaData } from "@/lib/dosha-data"
// import FloatingNavbar from "@/components/floating-navbar"
// import YogaPosesModal from "@/components/yoga-poses-modal"
// import { FloatingElements3D } from "@/components/3d-floating-elements"
// import { Lotus3D } from "@/components/3d-lotus"
// import { Chakra3D } from "@/components/3d-chakra"
// import { Footer } from "@/components/footer"
// import CustomCursor from "@/components/CustomCursor"
// import { AuroraBackground } from "@/components/aurora-background"


// // shadcn imports
// import { Card, CardContent, CardHeader, CardTitle } from "@/components/ui/card"
// import { Progress } from "@/components/ui/progress"
// import { Badge } from "@/components/ui/badge"
// import BreathingExercise from "@/components/BreathingExercise"
// import Chatbot from "@/components/Chatbot"

// export default function WellnessDashboardPage() {
//   const [hover, setHover] = useState(false)
//   const [doshaType, setDoshaType] = useState("")
//   const [doshaScores, setDoshaScores] = useState({})
//   const [modalContent, setModalContent] = useState({
//     isOpen: false,
//     title: "",
//     items: [],
//     icon: "",
//     gradient: "",
//   })
//   const [isYogaPosesOpen, setIsYogaPosesOpen] = useState(false)
//   const router = useRouter()

//   useEffect(() => {
//     const result = localStorage.getItem("doshaResult")
//     const scores = localStorage.getItem("doshaScores")

//     if (result && scores) {
//       setDoshaType(result)
//       setDoshaScores(JSON.parse(scores))
//     } else {
//       router.push("/dosha-quiz")
//     }
//   }, [router])

//   const currentDosha = doshaData[doshaType]

//   if (!currentDosha) return null

//   const handleRetakeQuiz = () => {
//     localStorage.removeItem("doshaResult")
//     localStorage.removeItem("doshaScores")
//     router.push("/dosha-quiz")
//   }

//   const openModal = (title, items, icon, gradient) => {
//     setModalContent({
//       isOpen: true,
//       title,
//       items,
//       icon,
//       gradient,
//     })
//   }

//   const closeModal = () => {
//     setModalContent((prev) => ({ ...prev, isOpen: false }))
//   }

//   const dashboardSections = [
//     {
//       key: "diet",
//       title: currentDosha.diet.title,
//       items: currentDosha.diet.items,
//       icon: "🥗",
//       gradient: "from-pink-300 to-rose-400",
//     },
//     {
//       key: "lifestyle",
//       title: currentDosha.lifestyle.title,
//       items: currentDosha.lifestyle.items,
//       icon: "🏠",
//       gradient: "from-pink-300 to-rose-400",
//     },
//     {
//       key: "yoga",
//       title: currentDosha.yoga.title,
//       items: currentDosha.yoga.items,
//       icon: "🧘‍♀️",
//       gradient: "from-pink-300 to-rose-400",
//     },
//     {
//       key: "remedies",
//       title: currentDosha.remedies.title,
//       items: currentDosha.remedies.items,
//       icon: "🌿",
//       gradient: "from-pink-300 to-rose-400",
//     },
//   ]

//   return (
//     <div className="min-h-screen bg-gradient-to-br from-pink-50 via-rose-100 to-pink-50">
//       <FloatingNavbar />
//       <AuroraBackground />
//       <FloatingElements3D />
//       <CustomCursor isHover={hover} />

//       <div className="relative z-10">
//         {/* Header */}
//         <div className="pt-20 pb-12 px-4">
//           <div className="max-w-6xl mx-auto text-center">
//             <ScrollReveal>
//               <GradientText className="text-4xl md:text-6xl font-serif font-bold mb-4">
//                 Your Wellness Dashboard
//               </GradientText>
//               <p className="text-xl text-gray-700 mb-8">
//                 Personalized recommendations for your {currentDosha.name} constitution
//               </p>
//             </ScrollReveal>

//             <ScrollReveal delay={200}>
//               <div className="flex justify-center items-center space-x-8 mb-8">
//                 <div className="text-center">
//                   <div className="text-2xl font-bold text-pink-500">{doshaScores.vata || 0}</div>
//                   <div className="text-sm text-gray-600">Vata</div>
//                 </div>
//                 <div className="text-center">
//                   <div className="text-2xl font-bold text-rose-500">{doshaScores.pitta || 0}</div>
//                   <div className="text-sm text-gray-600">Pitta</div>
//                 </div>
//                 <div className="text-center">
//                   <div className="text-2xl font-bold text-pink-600">{doshaScores.kapha || 0}</div>
//                   <div className="text-sm text-gray-600">Kapha</div>
//                 </div>
//               </div>
//             </ScrollReveal>

//             <ScrollReveal delay={400}>
//               <div className="flex flex-wrap justify-center gap-4">
//                 <button
//                   onClick={handleRetakeQuiz}
//                   className="px-6 py-3 glass-card rounded-xl text-gray-800 bg-pink-200/50 hover:bg-pink-300/70 transition-all duration-300 transform hover:scale-105"
//                 >
//                   Re-evaluate My Dosha
//                 </button>
//                 <button
//                   onClick={() => setIsYogaPosesOpen(true)}
//                   className="px-6 py-3 glass-card rounded-xl text-gray-800 bg-pink-200/50 hover:bg-pink-300/70 transition-all duration-300 transform hover:scale-105 flex items-center space-x-2"
//                 >
//                   <span>🧘‍♀️</span>
//                   <span>Explore Yoga Poses</span>
//                 </button>
//               </div>
//             </ScrollReveal>
//           </div>
//         </div>

//         {/* Daily Tracker + Streak Section */}
//         <ScrollReveal>
//           <div className="max-w-6xl mx-auto px-4 pb-20 grid md:grid-cols-2 gap-8">
//             {/* Daily Tracker */}
//             <Card className="glass-card rounded-3xl bg-pink-100/50">
//               <CardHeader>
//                 <CardTitle className="text-gray-800 font-serif text-xl">Daily Tracker</CardTitle>
//               </CardHeader>
//               <CardContent className="space-y-4">
//                 <div>
//                   <p className="text-gray-700 text-sm mb-2">Morning Meditation</p>
//                   <Progress value={30} className="h-2" />
//                 </div>
//                 <div>
//                   <p className="text-gray-700 text-sm mb-2">Yoga Practice</p>
//                   <Progress value={45} className="h-2" />
//                 </div>
//                 <div>
//                   <p className="text-gray-700 text-sm mb-2">Mindful Eating</p>
//                   <Progress value={15} className="h-2" />
//                 </div>
//               </CardContent>
//             </Card>

//             {/* Streak Section */}
//             <Card className="glass-card rounded-3xl bg-pink-100/50">
//               <CardHeader>
//                 <CardTitle className="text-gray-800 font-serif text-xl">Your Streak</CardTitle>
//               </CardHeader>
//               <CardContent className="flex flex-col items-center space-y-4">
//                 <div className="text-6xl font-bold text-pink-500">🔥</div>
//                 <p className="text-gray-600">Start your Streak</p>
//                 <div className="flex gap-2">
//                   {[...Array(7)].map((_, i) => (
//                     <Badge key={i} className={`rounded-full px-3 py-1 ${i < 7 ? "bg-gradient-to-r from-pink-400 to-rose-500 text-white" : "bg-gray-300"}`}>
//                       {i + 1}
//                     </Badge>
//                   ))}
//                 </div>
//               </CardContent>
//             </Card>
//           </div>
//         </ScrollReveal>

//         {/* Breathing Exercise Section */}
//         <ScrollReveal>
//           <div className="flex justify-center items-center pb-20">
//             <Card className="glass-card rounded-3xl p-6 w-[22rem] bg-pink-100/50">
//               <CardHeader>
//                 <CardTitle className="text-gray-800 font-serif text-xl text-center">
//                   Breathing Exercise
//                 </CardTitle>
//               </CardHeader>
//               <CardContent className="flex justify-start">
//                 <BreathingExercise />
//               </CardContent>
//             </Card>
//           </div>
//         </ScrollReveal>

//         {/* Recommendations Grid */}
// <div className="max-w-6xl mx-auto px-4 pb-20">
//   <div className="grid md:grid-cols-2 grid-cols-1 gap-8">
//     {dashboardSections.slice(0, 4).map((section, index) => (
//       <ScrollReveal key={section.key} delay={index * 100} animation="slide-up">
//         <div className="group relative">
//           <button
//             onClick={() => openModal(section.title, section.items, section.icon, section.gradient)}
//             className="w-full text-left glass-card rounded-3xl p-8 bg-pink-100/50 hover:bg-pink-200/70 transition-all duration-500 transform hover:scale-105 hover:-translate-y-2"
//           >
//             {/* Icon */}
//             <div className="mb-6">
//               <div
//                 className={`w-16 h-16 rounded-2xl bg-gradient-to-r ${section.gradient} flex items-center justify-center text-2xl mb-4 animate-pulse`}
//               >
//                 {section.icon}
//               </div>
//               <h3 className="text-2xl font-serif font-bold text-gray-800 mb-2">{section.title}</h3>
//             </div>

//             {/* Preview of first 2 items */}
//             <div className="space-y-3">
//               {section.items.slice(0, 2).map((item, itemIndex) => (
//                 <div
//                   key={itemIndex}
//                   className="flex items-start space-x-3 group-hover:translate-x-1 transition-transform duration-300"
//                   style={{ transitionDelay: `${itemIndex * 50}ms` }}
//                 >
//                   <div
//                     className={`w-2 h-2 rounded-full bg-gradient-to-r ${section.gradient} mt-2 flex-shrink-0`}
//                   />
//                   <p className="text-gray-700 text-sm leading-relaxed">{item}</p>
//                 </div>
//               ))}
//               {section.items.length > 2 && (
//                 <div className="text-center pt-4">
//                   <span className="text-gray-600 text-sm">Click to see all recommendations →</span>
//                 </div>
//               )}
//             </div>

//             {/* Hover Glow Effect */}
//             <div
//               className={`absolute inset-0 rounded-3xl bg-gradient-to-r ${section.gradient} opacity-0 group-hover:opacity-20 transition-opacity duration-500 pointer-events-none`}
//             />
//           </button>
//         </div>
//       </ScrollReveal>
//     ))}
//   </div>
// </div>

//         {/* Daily Practice Section */}
//         <ScrollReveal>
//           <div className="max-w-4xl mx-auto px-4 pb-20">
//             <div className="glass-card rounded-3xl p-8 bg-pink-100/50">
//               <div className="text-center mb-8">
//                 <GradientText className="text-3xl font-serif font-bold mb-4">Daily Practice Reminder</GradientText>
//                 <p className="text-gray-700">Consistency is key to balancing your {currentDosha.name} constitution</p>
//               </div>

//               <div className="grid md:grid-cols-3 gap-6">
//                 <div className="text-center">
//                   <div className="w-12 h-12 rounded-full bg-gradient-to-r from-pink-400 to-rose-500 flex items-center justify-center text-xl mb-3 mx-auto animate-spin-slow">
//                     🧘
//                   </div>
//                   <h4 className="font-semibold text-gray-800 mb-2">Morning Practice</h4>
//                   <p className="text-sm text-gray-600">Start with meditation and pranayama</p>
//                 </div>

//                 <div className="text-center">
//                   <div className="w-12 h-12 rounded-full bg-gradient-to-r from-rose-400 to-pink-500 flex items-center justify-center text-xl mb-3 mx-auto animate-bounce-slow">
//                     🍃
//                   </div>
//                   <h4 className="font-semibold text-gray-800 mb-2">Mindful Eating</h4>
//                   <p className="text-sm text-gray-600">Follow your dosha-specific diet</p>
//                 </div>

//                 <div className="text-center">
//                   <div className="w-12 h-12 rounded-full bg-gradient-to-r from-pink-500 to-rose-600 flex items-center justify-center text-xl mb-3 mx-auto animate-pulse">
//                     🌙
//                   </div>
//                   <h4 className="font-semibold text-gray-800 mb-2">Evening Routine</h4>
//                   <p className="text-sm text-gray-600">Wind down with gentle yoga</p>
//                 </div>
//               </div>
//             </div>
//           </div>
//         </ScrollReveal>
//       </div>

//       <ContentModal
//         isOpen={modalContent.isOpen}
//         onClose={closeModal}
//         title={modalContent.title}
//         items={modalContent.items}
//         icon={modalContent.icon}
//         gradient={modalContent.gradient}
//       />

//       <YogaPosesModal isOpen={isYogaPosesOpen} onClose={() => setIsYogaPosesOpen(false)} />

//       <div className="absolute top-32 left-10 opacity-50">
//         <Lotus3D />
//       </div>
//       <div className="absolute top-40 right-20 opacity-60">
//         <Chakra3D size={80} color="pink" speed={0.5} />
//       </div>
//       <div className="absolute bottom-40 left-1/4 opacity-40">
//         <Chakra3D size={60} color="rose" speed={1.2} />
//       </div>

//       <style jsx>{`
//         @keyframes spin-slow {
//           from { transform: rotate(0deg); }
//           to { transform: rotate(360deg); }
//         }

//         @keyframes bounce-slow {
//           0%, 100% { transform: translateY(0); }
//           50% { transform: translateY(-10px); }
//         }

//         .animate-spin-slow {
//           animation: spin-slow 8s linear infinite;
//         }

//         .animate-bounce-slow {
//           animation: bounce-slow 3s ease-in-out infinite;
//         }

//         .glass-card {
//           background: rgba(255, 255, 255, 0.5);
//           backdrop-filter: blur(10px);
//           border: 1px solid rgba(255, 192, 203, 0.3);
//         }
//       `}</style>

//       {/* Chatbot fixed at bottom-right */}
//       <Chatbot />
//       <Footer />
//     </div>
//   )
// }








"use client"

import { useEffect, useState } from "react"
import { useRouter } from "next/navigation"
import { AnimatedBackground } from "@/components/animated-background"
import { GradientText } from "@/components/gradient-text"
import { ScrollReveal } from "@/components/scroll-reveal"
import { ContentModal } from "@/components/content-modal"
import { doshaData } from "@/lib/dosha-data"
import FloatingNavbar from "@/components/floating-navbar"
import YogaPosesModal from "@/components/yoga-poses-modal"
import { FloatingElements3D } from "@/components/3d-floating-elements"
import { Lotus3D } from "@/components/3d-lotus"
import { Chakra3D } from "@/components/3d-chakra"
import { Footer } from "@/components/footer"
import CustomCursor from "@/components/CustomCursor"
import { AuroraBackground } from "@/components/aurora-background"

// shadcn imports
import { Card, CardContent, CardHeader, CardTitle } from "@/components/ui/card"
import { Progress } from "@/components/ui/progress"
import { Badge } from "@/components/ui/badge"
import BreathingExercise from "@/components/BreathingExercise"
import Chatbot from "@/components/Chatbot"

export default function WellnessDashboardPage() {
  const [hover, setHover] = useState(false)
  const [doshaType, setDoshaType] = useState("")
  const [doshaScores, setDoshaScores] = useState({})
  const [modalContent, setModalContent] = useState({
    isOpen: false,
    title: "",
    items: [],
    icon: "",
    gradient: "",
  })
  const [isYogaPosesOpen, setIsYogaPosesOpen] = useState(false)
  const [habits, setHabits] = useState([
    { id: "meditation", name: "Morning Meditation", value: 0 },
    { id: "yoga", name: "Yoga Practice", value: 0 },
    { id: "eating", name: "Mindful Eating", value: 0 },
  ])
  const [streak, setStreak] = useState(0)
  const router = useRouter()

  useEffect(() => {
    const result = localStorage.getItem("doshaResult")
    const scores = localStorage.getItem("doshaScores")
    const savedHabits = localStorage.getItem("habits")
    const savedStreak = localStorage.getItem("streak")

    if (result && scores) {
      setDoshaType(result)
      setDoshaScores(JSON.parse(scores))
    } else {
      router.push("/dosha-quiz")
    }
    if (savedHabits) setHabits(JSON.parse(savedHabits))
    if (savedStreak) setStreak(Number(savedStreak))
  }, [router])

  useEffect(() => {
    localStorage.setItem("habits", JSON.stringify(habits))
    localStorage.setItem("streak", String(streak))
  }, [habits, streak])

  const currentDosha = doshaData[doshaType]

  if (!currentDosha) return null

  const handleRetakeQuiz = () => {
    localStorage.removeItem("doshaResult")
    localStorage.removeItem("doshaScores")
    router.push("/dosha-quiz")
  }

  const openModal = (title, items, icon, gradient) => {
    setModalContent({
      isOpen: true,
      title,
      items,
      icon,
      gradient,
    })
  }

  const closeModal = () => {
    setModalContent((prev) => ({ ...prev, isOpen: false }))
  }

  const handleCheck = (id) => {
    const updatedHabits = habits.map((habit) =>
      habit.id === id ? { ...habit, value: habit.value === 0 ? 100 : 0 } : habit
    )
    setHabits(updatedHabits)

    const allDone = updatedHabits.every((h) => h.value === 100)
    if (allDone) {
      setStreak((prev) => prev + 1)
    }
  }

  const dashboardSections = [
    {
      key: "diet",
      title: currentDosha.diet.title,
      items: currentDosha.diet.items,
      icon: "🥗",
      gradient: "from-pink-300 to-rose-400",
    },
    {
      key: "lifestyle",
      title: currentDosha.lifestyle.title,
      items: currentDosha.lifestyle.items,
      icon: "🏠",
      gradient: "from-pink-300 to-rose-400",
    },
    {
      key: "yoga",
      title: currentDosha.yoga.title,
      items: currentDosha.yoga.items,
      icon: "🧘‍♀️",
      gradient: "from-pink-300 to-rose-400",
    },
    {
      key: "remedies",
      title: currentDosha.remedies.title,
      items: currentDosha.remedies.items,
      icon: "🌿",
      gradient: "from-pink-300 to-rose-400",
    },
  ]

  return (
    <div className="min-h-screen bg-gradient-to-br from-pink-50 via-rose-50 to-orange-50 py-20">
      <FloatingNavbar />
      <AuroraBackground />
      <FloatingElements3D />
      <CustomCursor isHover={hover} />

      <div className="relative z-10">
        {/* Header */}
        <ScrollReveal>
          <div className="text-center space-y-4">
            <GradientText className="text-5xl font-serif text-gray-800">
              Wellness Dashboard
            </GradientText>
            <p className="text-gray-600 text-lg">Track your journey and celebrate progress 🌸</p>
          </div>
        </ScrollReveal>

        <div className="max-w-6xl mx-auto px-4 pt-12">
          <ScrollReveal delay={200}>
            <div className="flex justify-center items-center space-x-8 mb-8">
              <div className="text-center">
                <div className="text-2xl font-bold text-pink-500">{doshaScores.vata || 0}</div>
                <div className="text-sm text-gray-600">Vata</div>
              </div>
              <div className="text-center">
                <div className="text-2xl font-bold text-rose-500">{doshaScores.pitta || 0}</div>
                <div className="text-sm text-gray-600">Pitta</div>
              </div>
              <div className="text-center">
                <div className="text-2xl font-bold text-pink-600">{doshaScores.kapha || 0}</div>
                <div className="text-sm text-gray-600">Kapha</div>
              </div>
            </div>
          </ScrollReveal>

          <ScrollReveal delay={400}>
            <div className="flex flex-wrap justify-center gap-4 mb-12">
              <button
                onClick={handleRetakeQuiz}
                className="px-6 py-3 glass-card rounded-xl text-gray-800 bg-pink-200/50 hover:bg-pink-300/70 transition-all duration-300 transform hover:scale-105"
              >
                Re-evaluate My Dosha
              </button>
              <button
                onClick={() => setIsYogaPosesOpen(true)}
                className="px-6 py-3 glass-card rounded-xl text-gray-800 bg-pink-200/50 hover:bg-pink-300/70 transition-all duration-300 transform hover:scale-105 flex items-center space-x-2"
              >
                <span>🧘‍♀️</span>
                <span>Explore Yoga Poses</span>
              </button>
            </div>
          </ScrollReveal>
        </div>

        {/* Daily Tracker + Streak Section */}
        <ScrollReveal>
          <div className="max-w-6xl mx-auto px-4 pb-20 grid md:grid-cols-2 gap-8">
            {/* Daily Tracker */}
            <Card className="glass-card rounded-3xl bg-pink-100/50">
              <CardHeader>
                <CardTitle className="text-gray-800 font-serif text-xl">Daily Tracker</CardTitle>
              </CardHeader>
              <CardContent className="space-y-4">
                {habits.map((habit) => (
                  <div key={habit.id} className="flex items-center gap-3">
                    <input
                      type="checkbox"
                      checked={habit.value === 100}
                      onChange={() => handleCheck(habit.id)}
                      className="w-5 h-5 accent-pink-500"
                    />
                    <div className="flex-1">
                      <p className="text-gray-700 text-sm mb-2">{habit.name}</p>
                      <Progress value={habit.value} className="h-2" />
                    </div>
                  </div>
                ))}
              </CardContent>
            </Card>

            {/* Streak Section */}
            <Card className="glass-card rounded-3xl bg-pink-100/50">
              <CardHeader>
                <CardTitle className="text-gray-800 font-serif text-xl">Your Streak</CardTitle>
              </CardHeader>
              <CardContent className="flex flex-col items-center space-y-4">
                <div className="text-6xl font-bold text-pink-500">🔥</div>
                <p className="text-gray-600">
                  {streak > 0 ? `You’re on a ${streak}-day streak!` : "Start your streak today"}
                </p>
                <div className="flex gap-2">
                  {[...Array(7)].map((_, i) => (
                    <Badge
                      key={i}
                      className={`rounded-full px-3 py-1 ${
                        i < streak % 7
                          ? "bg-gradient-to-r from-pink-400 to-rose-500 text-white"
                          : "bg-gray-300"
                      }`}
                    >
                      {i + 1}
                    </Badge>
                  ))}
                </div>
              </CardContent>
            </Card>
          </div>
        </ScrollReveal>

        {/* Breathing Exercise Section */}
        <ScrollReveal>
          <div className="flex justify-center items-center pb-20">
            <Card className="glass-card rounded-3xl p-6 w-[22rem] bg-pink-100/50">
              <CardHeader>
                <CardTitle className="text-gray-800 font-serif text-xl text-center">
                  Breathing Exercise
                </CardTitle>
              </CardHeader>
              <CardContent className="flex justify-start">
                <BreathingExercise />
              </CardContent>
            </Card>
          </div>
        </ScrollReveal>

        {/* Recommendations Grid */}
        <div className="max-w-6xl mx-auto px-4 pb-20">
          <div className="grid md:grid-cols-2 grid-cols-1 gap-8">
            {dashboardSections.map((section, index) => (
              <ScrollReveal key={section.key} delay={index * 100} animation="slide-up">
                <div className="group relative">
                  <button
                    onClick={() => openModal(section.title, section.items, section.icon, section.gradient)}
                    className="w-full text-left glass-card rounded-3xl p-8 bg-pink-100/50 hover:bg-pink-200/70 transition-all duration-500 transform hover:scale-105 hover:-translate-y-2"
                  >
                    {/* Icon */}
                    <div className="mb-6">
                      <div
                        className={`w-16 h-16 rounded-2xl bg-gradient-to-r ${section.gradient} flex items-center justify-center text-2xl mb-4 animate-pulse`}
                      >
                        {section.icon}
                      </div>
                      <h3 className="text-2xl font-serif font-bold text-gray-800 mb-2">{section.title}</h3>
                    </div>

                    {/* Preview of first 2 items */}
                    <div className="space-y-3">
                      {section.items.slice(0, 2).map((item, itemIndex) => (
                        <div
                          key={itemIndex}
                          className="flex items-start space-x-3 group-hover:translate-x-1 transition-transform duration-300"
                          style={{ transitionDelay: `${itemIndex * 50}ms` }}
                        >
                          <div
                            className={`w-2 h-2 rounded-full bg-gradient-to-r ${section.gradient} mt-2 flex-shrink-0`}
                          />
                          <p className="text-gray-700 text-sm leading-relaxed">{item}</p>
                        </div>
                      ))}
                      {section.items.length > 2 && (
                        <div className="text-center pt-4">
                          <span className="text-gray-600 text-sm">Click to see all recommendations →</span>
                        </div>
                      )}
                    </div>

                    {/* Hover Glow Effect */}
                    <div
                      className={`absolute inset-0 rounded-3xl bg-gradient-to-r ${section.gradient} opacity-0 group-hover:opacity-20 transition-opacity duration-500 pointer-events-none`}
                    />
                  </button>
                </div>
              </ScrollReveal>
            ))}
          </div>
        </div>

        {/* Daily Practice Section */}
        <ScrollReveal>
          <div className="max-w-4xl mx-auto px-4 pb-20">
            <div className="glass-card rounded-3xl p-8 bg-pink-100/50">
              <div className="text-center mb-8">
                <GradientText className="text-3xl font-serif font-bold mb-4">Daily Practice Reminder</GradientText>
                <p className="text-gray-700">Consistency is key to balancing your {currentDosha.name} constitution</p>
              </div>

              <div className="grid md:grid-cols-3 gap-6">
                <div className="text-center">
                  <div className="w-12 h-12 rounded-full bg-gradient-to-r from-pink-400 to-rose-500 flex items-center justify-center text-xl mb-3 mx-auto animate-spin-slow">
                    🧘
                  </div>
                  <h4 className="font-semibold text-gray-800 mb-2">Morning Practice</h4>
                  <p className="text-sm text-gray-600">Start with meditation and pranayama</p>
                </div>

                <div className="text-center">
                  <div className="w-12 h-12 rounded-full bg-gradient-to-r from-rose-400 to-pink-500 flex items-center justify-center text-xl mb-3 mx-auto animate-bounce-slow">
                    🍃
                  </div>
                  <h4 className="font-semibold text-gray-800 mb-2">Mindful Eating</h4>
                  <p className="text-sm text-gray-600">Follow your dosha-specific diet</p>
                </div>

                <div className="text-center">
                  <div className="w-12 h-12 rounded-full bg-gradient-to-r from-pink-500 to-rose-600 flex items-center justify-center text-xl mb-3 mx-auto animate-pulse">
                    🌙
                  </div>
                  <h4 className="font-semibold text-gray-800 mb-2">Evening Routine</h4>
                  <p className="text-sm text-gray-600">Wind down with gentle yoga</p>
                </div>
              </div>
            </div>
          </div>
        </ScrollReveal>
      </div>

      <ContentModal
        isOpen={modalContent.isOpen}
        onClose={closeModal}
        title={modalContent.title}
        items={modalContent.items}
        icon={modalContent.icon}
        gradient={modalContent.gradient}
      />

      <YogaPosesModal isOpen={isYogaPosesOpen} onClose={() => setIsYogaPosesOpen(false)} />

      <div className="absolute top-32 left-10 opacity-50">
        <Lotus3D />
      </div>
      <div className="absolute top-40 right-20 opacity-60">
        <Chakra3D size={80} color="pink" speed={0.5} />
      </div>
      <div className="absolute bottom-40 left-1/4 opacity-40">
        <Chakra3D size={60} color="rose" speed={1.2} />
      </div>

      <style jsx>{`
        @keyframes spin-slow {
          from { transform: rotate(0deg); }
          to { transform: rotate(360deg); }
        }

        @keyframes bounce-slow {
          0%, 100% { transform: translateY(0); }
          50% { transform: translateY(-10px); }
        }

        .animate-spin-slow {
          animation: spin-slow 8s linear infinite;
        }

        .animate-bounce-slow {
          animation: bounce-slow 3s ease-in-out infinite;
        }

        .glass-card {
          background: rgba(255, 255, 255, 0.5);
          backdrop-filter: blur(10px);
          border: 1px solid rgba(255, 192, 203, 0.3);
        }
      `}</style>

      {/* Chatbot fixed at bottom-right */}
      <Chatbot />
      <Footer />
    </div>
  )
}




// // "use client"

// // import { useEffect, useState } from "react"
// // import { useRouter } from "next/navigation"
// // import { AnimatedBackground } from "@/components/animated-background"
// // import { GradientText } from "@/components/gradient-text"
// // import { ScrollReveal } from "@/components/scroll-reveal"
// // import { ContentModal } from "@/components/content-modal"
// // import { doshaData } from "@/lib/dosha-data"
// // import FloatingNavbar from "@/components/floating-navbar"
// // import YogaPosesModal from "@/components/yoga-poses-modal"
// // import { FloatingElements3D } from "@/components/3d-floating-elements"
// // import { Lotus3D } from "@/components/3d-lotus"
// // import { Chakra3D } from "@/components/3d-chakra"
// // import { Footer } from "@/components/footer"
// // import CustomCursor from "@/components/CustomCursor"
// // import { AuroraBackground } from "@/components/aurora-background"

// // // shadcn imports
// // import { Card, CardContent, CardHeader, CardTitle } from "@/components/ui/card"
// // import { Progress } from "@/components/ui/progress"
// // import { Badge } from "@/components/ui/badge"
// // import BreathingExercise from "@/components/BreathingExercise"
// // import Chatbot from "@/components/Chatbot"

// // export default function WellnessDashboardPage() {
// //   const [hover, setHover] = useState(false)
// //   const [doshaType, setDoshaType] = useState("")
// //   const [doshaScores, setDoshaScores] = useState({})
// //   const [modalContent, setModalContent] = useState({
// //     isOpen: false,
// //     title: "",
// //     items: [],
// //     icon: "",
// //     gradient: "",
// //   })
// //   const [isYogaPosesOpen, setIsYogaPosesOpen] = useState(false)
// //   const [quizStep, setQuizStep] = useState(0)
// //   const [answers, setAnswers] = useState([])
// //   const [feedback, setFeedback] = useState("")
// //   const router = useRouter()

// //   useEffect(() => {
// //     const result = localStorage.getItem("doshaResult")
// //     const scores = localStorage.getItem("doshaScores")

// //     if (result && scores) {
// //       setDoshaType(result)
// //       setDoshaScores(JSON.parse(scores))
// //     } else {
// //       router.push("/dosha-quiz")
// //     }
// //   }, [router])

// //   const currentDosha = doshaData[doshaType]

// //   if (!currentDosha) return null

// //   const handleRetakeQuiz = () => {
// //     localStorage.removeItem("doshaResult")
// //     localStorage.removeItem("doshaScores")
// //     router.push("/dosha-quiz")
// //   }

// //   const openModal = (title, items, icon, gradient) => {
// //     setModalContent({
// //       isOpen: true,
// //       title,
// //       items,
// //       icon,
// //       gradient,
// //     })
// //     if (title === "Take a Quick Quiz") {
// //       setQuizStep(0)
// //       setAnswers([])
// //       setFeedback("")
// //     }
// //   }

// //   const closeModal = () => {
// //     setModalContent((prev) => ({ ...prev, isOpen: false }))
// //     setQuizStep(0)
// //     setAnswers([])
// //     setFeedback("")
// //   }

// //   const handleAnswer = (answer) => {
// //     setAnswers([...answers, answer])
// //     if (quizStep < quizQuestions.length - 1) {
// //       setQuizStep(quizStep + 1)
// //     } else {
// //       calculateFeedback()
// //       setQuizStep(0) // Reset for next use
// //     }
// //   }

// //   const calculateFeedback = () => {
// //     const vataCount = answers.filter(a => a.includes("restless") || a.includes("scattered")).length
// //     const pittaCount = answers.filter(a => a.includes("heated") || a.includes("irritated")).length
// //     const kaphaCount = answers.filter(a => a.includes("slow") || a.includes("sluggish")).length

// //     const maxCount = Math.max(vataCount, pittaCount, kaphaCount)
// //     if (maxCount === vataCount) {
// //       setFeedback("Your responses suggest a Vata imbalance. Try grounding activities like warm meals and gentle yoga.")
// //     } else if (maxCount === pittaCount) {
// //       setFeedback("Your responses suggest a Pitta imbalance. Consider cooling practices and avoiding spicy foods.")
// //     } else if (maxCount === kaphaCount) {
// //       setFeedback("Your responses suggest a Kapha imbalance. Engage in stimulating exercises and light foods.")
// //     } else {
// //       setFeedback("Your responses are balanced! Maintain your current routine with minor adjustments.")
// //     }
// //   }

// //   const quizQuestions = [
// //     {
// //       question: "How would you describe your energy levels today?",
// //       options: ["Low and sluggish", "Moderate and steady", "High and restless"],
// //     },
// //     {
// //       question: "How do you react to stress?",
// //       options: ["I feel anxious or scattered", "I get irritated or heated", "I feel calm but slow"],
// //     },
// //   ]

// //   const dashboardSections = [
// //     {
// //       key: "diet",
// //       title: currentDosha.diet.title,
// //       items: currentDosha.diet.items,
// //       icon: "🥗",
// //       gradient: "from-pink-300 to-rose-400",
// //     },
// //     {
// //       key: "lifestyle",
// //       title: currentDosha.lifestyle.title,
// //       items: currentDosha.lifestyle.items,
// //       icon: "🏠",
// //       gradient: "from-pink-300 to-rose-400",
// //     },
// //     {
// //       key: "yoga",
// //       title: currentDosha.yoga.title,
// //       items: currentDosha.yoga.items,
// //       icon: "🧘‍♀️",
// //       gradient: "from-pink-300 to-rose-400",
// //     },
// //     {
// //       key: "remedies",
// //       title: currentDosha.remedies.title,
// //       items: currentDosha.remedies.items,
// //       icon: "🌿",
// //       gradient: "from-pink-300 to-rose-400",
// //     },
// //     {
// //       key: "quiz",
// //       title: "Take a Quick Quiz",
// //       items: ["Answer a few questions", "Get personalized feedback"],
// //       icon: "📝",
// //       gradient: "from-pink-300 to-rose-400",
// //     },
// //   ]

// //   return (
// //     <div className="min-h-screen bg-gradient-to-br from-pink-50 via-rose-100 to-pink-50">
// //       <FloatingNavbar />
// //       <AuroraBackground />
// //       <FloatingElements3D />
// //       <CustomCursor isHover={hover} />

// //       <div className="relative z-10">
// //         {/* Header */}
// //         <div className="pt-20 pb-12 px-4">
// //           <div className="max-w-6xl mx-auto text-center">
// //             <ScrollReveal>
// //               <GradientText className="text-4xl md:text-6xl font-serif font-bold mb-4">
// //                 Your Wellness Dashboard
// //               </GradientText>
// //               <p className="text-xl text-gray-700 mb-8">
// //                 Personalized recommendations for your {currentDosha.name} constitution
// //               </p>
// //             </ScrollReveal>

// //             <ScrollReveal delay={200}>
// //               <div className="flex justify-center items-center space-x-8 mb-8">
// //                 <div className="text-center">
// //                   <div className="text-2xl font-bold text-pink-500">{doshaScores.vata || 0}</div>
// //                   <div className="text-sm text-gray-600">Vata</div>
// //                 </div>
// //                 <div className="text-center">
// //                   <div className="text-2xl font-bold text-rose-500">{doshaScores.pitta || 0}</div>
// //                   <div className="text-sm text-gray-600">Pitta</div>
// //                 </div>
// //                 <div className="text-center">
// //                   <div className="text-2xl font-bold text-pink-600">{doshaScores.kapha || 0}</div>
// //                   <div className="text-sm text-gray-600">Kapha</div>
// //                 </div>
// //               </div>
// //             </ScrollReveal>

// //             <ScrollReveal delay={400}>
// //               <div className="flex flex-wrap justify-center gap-4">
// //                 <button
// //                   onClick={handleRetakeQuiz}
// //                   className="px-6 py-3 glass-card rounded-xl text-gray-800 bg-pink-200/50 hover:bg-pink-300/70 transition-all duration-300 transform hover:scale-105"
// //                 >
// //                   Re-evaluate My Dosha
// //                 </button>
// //                 <button
// //                   onClick={() => setIsYogaPosesOpen(true)}
// //                   className="px-6 py-3 glass-card rounded-xl text-gray-800 bg-pink-200/50 hover:bg-pink-300/70 transition-all duration-300 transform hover:scale-105 flex items-center space-x-2"
// //                 >
// //                   <span>🧘‍♀️</span>
// //                   <span>Explore Yoga Poses</span>
// //                 </button>
// //               </div>
// //             </ScrollReveal>
// //           </div>
// //         </div>

// //         {/* Daily Tracker + Streak Section */}
// //         <ScrollReveal>
// //           <div className="max-w-6xl mx-auto px-4 pb-20 grid md:grid-cols-2 gap-8">
// //             {/* Daily Tracker */}
// //             <Card className="glass-card rounded-3xl bg-pink-100/50">
// //               <CardHeader>
// //                 <CardTitle className="text-gray-800 font-serif text-xl">Daily Tracker</CardTitle>
// //               </CardHeader>
// //               <CardContent className="space-y-4">
// //                 <div>
// //                   <p className="text-gray-700 text-sm mb-2">Morning Meditation</p>
// //                   <Progress value={30} className="h-2" />
// //                 </div>
// //                 <div>
// //                   <p className="text-gray-700 text-sm mb-2">Yoga Practice</p>
// //                   <Progress value={45} className="h-2" />
// //                 </div>
// //                 <div>
// //                   <p className="text-gray-700 text-sm mb-2">Mindful Eating</p>
// //                   <Progress value={15} className="h-2" />
// //                 </div>
// //               </CardContent>
// //             </Card>

// //             {/* Streak Section */}
// //             <Card className="glass-card rounded-3xl bg-pink-100/50">
// //               <CardHeader>
// //                 <CardTitle className="text-gray-800 font-serif text-xl">Your Streak</CardTitle>
// //               </CardHeader>
// //               <CardContent className="flex flex-col items-center space-y-4">
// //                 <div className="text-6xl font-bold text-pink-500">🔥</div>
// //                 <p className="text-gray-600">Start your Streak</p>
// //                 <div className="flex gap-2">
// //                   {[...Array(7)].map((_, i) => (
// //                     <Badge key={i} className={`rounded-full px-3 py-1 ${i < 7 ? "bg-gradient-to-r from-pink-400 to-rose-500 text-white" : "bg-gray-300"}`}>
// //                       {i + 1}
// //                     </Badge>
// //                   ))}
// //                 </div>
// //               </CardContent>
// //             </Card>
// //           </div>
// //         </ScrollReveal>

// //         {/* Breathing Exercise Section */}
// //         <ScrollReveal>
// //           <div className="flex justify-center items-center pb-20">
// //             <Card className="glass-card rounded-3xl p-6 w-[22rem] bg-pink-100/50">
// //               <CardHeader>
// //                 <CardTitle className="text-gray-800 font-serif text-xl text-center">
// //                   Breathing Exercise
// //                 </CardTitle>
// //               </CardHeader>
// //               <CardContent className="flex justify-start">
// //                 <BreathingExercise />
// //               </CardContent>
// //             </Card>
// //           </div>
// //         </ScrollReveal>

// //         {/* Recommendations Grid */}
// //         <div className="max-w-6xl mx-auto px-4 pb-20">
// //           <div className="grid md:grid-cols-2 grid-cols-1 gap-8">
// //             {dashboardSections.slice(0, 4).map((section, index) => (
// //               <ScrollReveal key={section.key} delay={index * 100} animation="slide-up">
// //                 <div className="group relative">
// //                   <button
// //                     onClick={() => openModal(section.title, section.items, section.icon, section.gradient)}
// //                     className="w-full text-left glass-card rounded-3xl p-8 bg-pink-100/50 hover:bg-pink-200/70 transition-all duration-500 transform hover:scale-105 hover:-translate-y-2"
// //                   >
// //                     {/* Icon */}
// //                     <div className="mb-6">
// //                       <div
// //                         className={`w-16 h-16 rounded-2xl bg-gradient-to-r ${section.gradient} flex items-center justify-center text-2xl mb-4 animate-pulse`}
// //                       >
// //                         {section.icon}
// //                       </div>
// //                       <h3 className="text-2xl font-serif font-bold text-gray-800 mb-2">{section.title}</h3>
// //                     </div>

// //                     {/* Preview of first 2 items */}
// //                     <div className="space-y-3">
// //                       {section.items.slice(0, 2).map((item, itemIndex) => (
// //                         <div
// //                           key={itemIndex}
// //                           className="flex items-start space-x-3 group-hover:translate-x-1 transition-transform duration-300"
// //                           style={{ transitionDelay: `${itemIndex * 50}ms` }}
// //                         >
// //                           <div
// //                             className={`w-2 h-2 rounded-full bg-gradient-to-r ${section.gradient} mt-2 flex-shrink-0`}
// //                           />
// //                           <p className="text-gray-700 text-sm leading-relaxed">{item}</p>
// //                         </div>
// //                       ))}
// //                       {section.items.length > 2 && (
// //                         <div className="text-center pt-4">
// //                           <span className="text-gray-600 text-sm">Click to see all recommendations →</span>
// //                         </div>
// //                       )}
// //                     </div>

// //                     {/* Hover Glow Effect */}
// //                     <div
// //                       className={`absolute inset-0 rounded-3xl bg-gradient-to-r ${section.gradient} opacity-0 group-hover:opacity-20 transition-opacity duration-500 pointer-events-none`}
// //                     />
// //                   </button>
// //                 </div>
// //               </ScrollReveal>
// //             ))}
// //           </div>
// //         </div>

// //         {/* Daily Practice Section */}
// //         <ScrollReveal>
// //           <div className="max-w-4xl mx-auto px-4 pb-20">
// //             <div className="glass-card rounded-3xl p-8 bg-pink-100/50">
// //               <div className="text-center mb-8">
// //                 <GradientText className="text-3xl font-serif font-bold mb-4">Daily Practice Reminder</GradientText>
// //                 <p className="text-gray-700">Consistency is key to balancing your {currentDosha.name} constitution</p>
// //               </div>

// //               <div className="grid md:grid-cols-3 gap-6">
// //                 <div className="text-center">
// //                   <div className="w-12 h-12 rounded-full bg-gradient-to-r from-pink-400 to-rose-500 flex items-center justify-center text-xl mb-3 mx-auto animate-spin-slow">
// //                     🧘
// //                   </div>
// //                   <h4 className="font-semibold text-gray-800 mb-2">Morning Practice</h4>
// //                   <p className="text-sm text-gray-600">Start with meditation and pranayama</p>
// //                 </div>

// //                 <div className="text-center">
// //                   <div className="w-12 h-12 rounded-full bg-gradient-to-r from-rose-400 to-pink-500 flex items-center justify-center text-xl mb-3 mx-auto animate-bounce-slow">
// //                     🍃
// //                   </div>
// //                   <h4 className="font-semibold text-gray-800 mb-2">Mindful Eating</h4>
// //                   <p className="text-sm text-gray-600">Follow your dosha-specific diet</p>
// //                 </div>

// //                 <div className="text-center">
// //                   <div className="w-12 h-12 rounded-full bg-gradient-to-r from-pink-500 to-rose-600 flex items-center justify-center text-xl mb-3 mx-auto animate-pulse">
// //                     🌙
// //                   </div>
// //                   <h4 className="font-semibold text-gray-800 mb-2">Evening Routine</h4>
// //                   <p className="text-sm text-gray-600">Wind down with gentle yoga</p>
// //                 </div>
// //               </div>
// //             </div>
// //           </div>
// //         </ScrollReveal>
// //       </div>

// //       {/* Content Modal */}
// //       <ContentModal
// //         isOpen={modalContent.isOpen}
// //         onClose={closeModal}
// //         title={modalContent.title}
// //         items={modalContent.items}
// //         icon={modalContent.icon}
// //         gradient={modalContent.gradient}
// //       >
// //         {modalContent.title === "Take a Quick Quiz" && quizStep < quizQuestions.length && (
// //           <div className="space-y-4">
// //             <h3 className="text-xl font-bold text-gray-800">{quizQuestions[quizStep].question}</h3>
// //             {quizQuestions[quizStep].options.map((option, idx) => (
// //               <button
// //                 key={idx}
// //                 onClick={() => handleAnswer(option)}
// //                 className="w-full py-2 px-4 rounded-lg bg-gradient-to-r from-pink-300 to-rose-400 text-white text-sm font-medium hover:bg-opacity-90 transition"
// //               >
// //                 {option}
// //               </button>
// //             ))}
// //           </div>
// //         )}
// //         {modalContent.title === "Take a Quick Quiz" && quizStep >= quizQuestions.length && feedback && (
// //           <div className="space-y-4 text-center">
// //             <h3 className="text-xl font-bold text-gray-800">Your Feedback</h3>
// //             <p className="text-gray-700">{feedback}</p>
// //             <button
// //               onClick={closeModal}
// //               className="w-full py-2 px-4 rounded-lg bg-gradient-to-r from-pink-300 to-rose-400 text-white text-sm font-medium hover:bg-opacity-90 transition"
// //             >
// //               Close
// //             </button>
// //           </div>
// //         )}
// //       </ContentModal>

// //       <YogaPosesModal isOpen={isYogaPosesOpen} onClose={() => setIsYogaPosesOpen(false)} />

// //       <div className="absolute top-32 left-10 opacity-50">
// //         <Lotus3D />
// //       </div>
// //       <div className="absolute top-40 right-20 opacity-60">
// //         <Chakra3D size={80} color="pink" speed={0.5} />
// //       </div>
// //       <div className="absolute bottom-40 left-1/4 opacity-40">
// //         <Chakra3D size={60} color="rose" speed={1.2} />
// //       </div>

// //       <style jsx>{`
// //         @keyframes spin-slow {
// //           from { transform: rotate(0deg); }
// //           to { transform: rotate(360deg); }
// //         }

// //         @keyframes bounce-slow {
// //           0%, 100% { transform: translateY(0); }
// //           50% { transform: translateY(-10px); }
// //         }

// //         .animate-spin-slow {
// //           animation: spin-slow 8s linear infinite;
// //         }

// //         .animate-bounce-slow {
// //           animation: bounce-slow 3s ease-in-out infinite;
// //         }

// //         .glass-card {
// //           background: rgba(255, 255, 255, 0.5);
// //           backdrop-filter: blur(10px);
// //           border: 1px solid rgba(255, 192, 203, 0.3);
// //         }
// //       `}</style>

// //       {/* Chatbot fixed at bottom-right */}
// //       <Chatbot />
// //       <Footer />
// //     </div>
// //   )
// // }




















// "use client"

// import { useEffect, useState } from "react"
// import { useRouter } from "next/navigation"
// import { AnimatedBackground } from "@/components/animated-background"
// import { GradientText } from "@/components/gradient-text"
// // Assuming ScrollReveal is a placeholder; replace with actual implementation
// const ScrollReveal = ({ children, delay, animation }) => <div>{children}</div>
// // Assuming ContentModal is a placeholder; replace with actual implementation
// const ContentModal = ({ isOpen, onClose, title, items, icon, gradient, children }) => (
//   isOpen ? <div>{children || <p>{title}: {items.join(", ")} {icon}</p>}<button onClick={onClose}>Close</button></div> : null
// )
// import { doshaData } from "@/lib/dosha-data"
// import FloatingNavbar from "@/components/floating-navbar"
// // Assuming YogaPosesModal is a placeholder; replace with actual implementation
// const YogaPosesModal = ({ isOpen, onClose }) => (isOpen ? <div><button onClick={onClose}>Close</button></div> : null)
// import { FloatingElements3D } from "@/components/3d-floating-elements"
// import { Lotus3D } from "@/components/3d-lotus"
// import { Chakra3D } from "@/components/3d-chakra"
// import { Footer } from "@/components/footer"
// import CustomCursor from "@/components/CustomCursor"
// import { AuroraBackground } from "@/components/aurora-background"

// // shadcn imports
// import { Card, CardContent, CardHeader, CardTitle } from "@/components/ui/card"
// import { Progress } from "@/components/ui/progress"
// import { Badge } from "@/components/ui/badge"
// import BreathingExercise from "@/components/BreathingExercise"
// import Chatbot from "@/components/Chatbot"

// // Type definitions
// interface Dosha {
//   name: string
//   diet: { title: string; items: string[] }
//   lifestyle: { title: string; items: string[] }
//   yoga: { title: string; items: string[] }
//   remedies: { title: string; items: string[] }
// }

// interface ModalContent {
//   isOpen: boolean
//   title: string
//   items: string[]
//   icon: string
//   gradient: string
// }

// interface DashboardSection {
//   key: string
//   title: string
//   items: string[]
//   icon: string
//   gradient: string
// }

// export default function WellnessDashboardPage() {
//   const [hover, setHover] = useState(false)
//   const [doshaType, setDoshaType] = useState<string>("")
//   const [doshaScores, setDoshaScores] = useState<Record<string, number>>({})
//   const [modalContent, setModalContent] = useState<ModalContent>({
//     isOpen: false,
//     title: "",
//     items: [],
//     icon: "",
//     gradient: "",
//   })
//   const [isYogaPosesOpen, setIsYogaPosesOpen] = useState(false)
//   const [quizStep, setQuizStep] = useState(0)
//   const [answers, setAnswers] = useState<string[]>([])
//   const [feedback, setFeedback] = useState<string>("")
//   const router = useRouter()

//   useEffect(() => {
//     const result = localStorage.getItem("doshaResult")
//     const scores = localStorage.getItem("doshaScores")

//     if (result && scores) {
//       try {
//         setDoshaType(result)
//         setDoshaScores(JSON.parse(scores))
//       } catch (e) {
//         console.error("Error parsing localStorage data:", e)
//         router.push("/dosha-quiz")
//       }
//     } else {
//       router.push("/dosha-quiz")
//     }
//   }, [router])

//   const currentDosha = doshaData[doshaType] as Dosha | undefined

//   if (!currentDosha) return null

//   const handleRetakeQuiz = () => {
//     localStorage.removeItem("doshaResult")
//     localStorage.removeItem("doshaScores")
//     router.push("/dosha-quiz")
//   }

//   const openModal = (title: string, items: string[], icon: string, gradient: string) => {
//     setModalContent({
//       isOpen: true,
//       title,
//       items,
//       icon,
//       gradient,
//     })
//     if (title === "Take a Quick Quiz") {
//       setQuizStep(0)
//       setAnswers([])
//       setFeedback("")
//     }
//   }

//   const closeModal = () => {
//     setModalContent((prev) => ({ ...prev, isOpen: false }))
//     setQuizStep(0)
//     setAnswers([])
//     setFeedback("")
//   }

//   const handleAnswer = (answer: string) => {
//     setAnswers([...answers, answer])
//     if (quizStep < quizQuestions.length - 1) {
//       setQuizStep(quizStep + 1)
//     } else {
//       calculateFeedback()
//       setQuizStep(0) // Reset for next use
//     }
//   }

//   const calculateFeedback = () => {
//     const vataCount = answers.filter(a => a.includes("restless") || a.includes("scattered")).length
//     const pittaCount = answers.filter(a => a.includes("heated") || a.includes("irritated")).length
//     const kaphaCount = answers.filter(a => a.includes("slow") || a.includes("sluggish")).length

//     const maxCount = Math.max(vataCount, pittaCount, kaphaCount)
//     if (maxCount === vataCount) {
//       setFeedback("Your responses suggest a Vata imbalance. Try grounding activities like warm meals and gentle yoga.")
//     } else if (maxCount === pittaCount) {
//       setFeedback("Your responses suggest a Pitta imbalance. Consider cooling practices and avoiding spicy foods.")
//     } else if (maxCount === kaphaCount) {
//       setFeedback("Your responses suggest a Kapha imbalance. Engage in stimulating exercises and light foods.")
//     } else {
//       setFeedback("Your responses are balanced! Maintain your current routine with minor adjustments.")
//     }
//   }

//   const quizQuestions = [
//     {
//       question: "How would you describe your energy levels today?",
//       options: ["Low and sluggish", "Moderate and steady", "High and restless"],
//     },
//     {
//       question: "How do you react to stress?",
//       options: ["I feel anxious or scattered", "I get irritated or heated", "I feel calm but slow"],
//     },
//   ]

//   const dashboardSections: DashboardSection[] = [
//     {
//       key: "diet",
//       title: currentDosha.diet.title,
//       items: currentDosha.diet.items,
//       icon: "🥗",
//       gradient: "from-pink-300 to-rose-400",
//     },
//     {
//       key: "lifestyle",
//       title: currentDosha.lifestyle.title,
//       items: currentDosha.lifestyle.items,
//       icon: "🏠",
//       gradient: "from-pink-300 to-rose-400",
//     },
//     {
//       key: "yoga",
//       title: currentDosha.yoga.title,
//       items: currentDosha.yoga.items,
//       icon: "🧘‍♀️",
//       gradient: "from-pink-300 to-rose-400",
//     },
//     {
//       key: "remedies",
//       title: currentDosha.remedies.title,
//       items: currentDosha.remedies.items,
//       icon: "🌿",
//       gradient: "from-pink-300 to-rose-400",
//     },
//     {
//       key: "quiz",
//       title: "Take a Quick Quiz",
//       items: ["Answer a few questions", "Get personalized feedback"],
//       icon: "📝",
//       gradient: "from-pink-300 to-rose-400",
//     },
//   ]

//   return (
//     <div className="min-h-screen bg-gradient-to-br from-pink-50 via-rose-100 to-pink-50">
//       <FloatingNavbar />
//       <AuroraBackground />
//       <FloatingElements3D />
//       <CustomCursor isHover={hover} />

//       <div className="relative z-10">
//         {/* Header */}
//         <div className="pt-20 pb-12 px-4">
//           <div className="max-w-6xl mx-auto text-center">
//             <ScrollReveal>
//               <GradientText className="text-4xl md:text-6xl font-serif font-bold mb-4">
//                 Your Wellness Dashboard
//               </GradientText>
//               <p className="text-xl text-gray-700 mb-8">
//                 Personalized recommendations for your {currentDosha.name} constitution
//               </p>
//             </ScrollReveal>

//             <ScrollReveal delay={200}>
//               <div className="flex justify-center items-center space-x-8 mb-8">
//                 <div className="text-center">
//                   <div className="text-2xl font-bold text-pink-500">{doshaScores.vata || 0}</div>
//                   <div className="text-sm text-gray-600">Vata</div>
//                 </div>
//                 <div className="text-center">
//                   <div className="text-2xl font-bold text-rose-500">{doshaScores.pitta || 0}</div>
//                   <div className="text-sm text-gray-600">Pitta</div>
//                 </div>
//                 <div className="text-center">
//                   <div className="text-2xl font-bold text-pink-600">{doshaScores.kapha || 0}</div>
//                   <div className="text-sm text-gray-600">Kapha</div>
//                 </div>
//               </div>
//             </ScrollReveal>

//             <ScrollReveal delay={400}>
//               <div className="flex flex-wrap justify-center gap-4">
//                 <button
//                   onClick={handleRetakeQuiz}
//                   className="px-6 py-3 glass-card rounded-xl text-gray-800 bg-pink-200/50 hover:bg-pink-300/70 transition-all duration-300 transform hover:scale-105"
//                 >
//                   Re-evaluate My Dosha
//                 </button>
//                 <button
//                   onClick={() => setIsYogaPosesOpen(true)}
//                   className="px-6 py-3 glass-card rounded-xl text-gray-800 bg-pink-200/50 hover:bg-pink-300/70 transition-all duration-300 transform hover:scale-105 flex items-center space-x-2"
//                 >
//                   <span>🧘‍♀️</span>
//                   <span>Explore Yoga Poses</span>
//                 </button>
//               </div>
//             </ScrollReveal>
//           </div>
//         </div>

//         {/* Daily Tracker + Streak Section */}
//         <ScrollReveal>
//           <div className="max-w-6xl mx-auto px-4 pb-20 grid md:grid-cols-2 gap-8">
//             {/* Daily Tracker */}
//             <Card className="glass-card rounded-3xl bg-pink-100/50">
//               <CardHeader>
//                 <CardTitle className="text-gray-800 font-serif text-xl">Daily Tracker</CardTitle>
//               </CardHeader>
//               <CardContent className="space-y-4">
//                 <div>
//                   <p className="text-gray-700 text-sm mb-2">Morning Meditation</p>
//                   <Progress value={30} className="h-2" />
//                 </div>
//                 <div>
//                   <p className="text-gray-700 text-sm mb-2">Yoga Practice</p>
//                   <Progress value={45} className="h-2" />
//                 </div>
//                 <div>
//                   <p className="text-gray-700 text-sm mb-2">Mindful Eating</p>
//                   <Progress value={15} className="h-2" />
//                 </div>
//               </CardContent>
//             </Card>

//             {/* Streak Section */}
//             <Card className="glass-card rounded-3xl bg-pink-100/50">
//               <CardHeader>
//                 <CardTitle className="text-gray-800 font-serif text-xl">Your Streak</CardTitle>
//               </CardHeader>
//               <CardContent className="flex flex-col items-center space-y-4">
//                 <div className="text-6xl font-bold text-pink-500">🔥</div>
//                 <p className="text-gray-600">Start your Streak</p>
//                 <div className="flex gap-2">
//                   {[...Array(7)].map((_, i) => (
//                     <Badge key={i} className={`rounded-full px-3 py-1 ${i < 7 ? "bg-gradient-to-r from-pink-400 to-rose-500 text-white" : "bg-gray-300"}`}>
//                       {i + 1}
//                     </Badge>
//                   ))}
//                 </div>
//               </CardContent>
//             </Card>
//           </div>
//         </ScrollReveal>

//         {/* Breathing Exercise Section */}
//         <ScrollReveal>
//           <div className="flex justify-center items-center pb-20">
//             <Card className="glass-card rounded-3xl p-6 w-[22rem] bg-pink-100/50">
//               <CardHeader>
//                 <CardTitle className="text-gray-800 font-serif text-xl text-center">
//                   Breathing Exercise
//                 </CardTitle>
//               </CardHeader>
//               <CardContent className="flex justify-start">
//                 <BreathingExercise />
//               </CardContent>
//             </Card>
//           </div>
//         </ScrollReveal>

//         {/* Recommendations Grid */}
//         <div className="max-w-6xl mx-auto px-4 pb-20">
//           <div className="grid md:grid-cols-2 grid-cols-1 gap-8">
//             {dashboardSections.slice(0, 4).map((section, index) => (
//               <ScrollReveal key={section.key} delay={index * 100} animation="slide-up">
//                 <div className="group relative">
//                   <button
//                     onClick={() => openModal(section.title, section.items, section.icon, section.gradient)}
//                     className="w-full text-left glass-card rounded-3xl p-8 bg-pink-100/50 hover:bg-pink-200/70 transition-all duration-500 transform hover:scale-105 hover:-translate-y-2"
//                   >
//                     {/* Icon */}
//                     <div className="mb-6">
//                       <div
//                         className={`w-16 h-16 rounded-2xl bg-gradient-to-r ${section.gradient} flex items-center justify-center text-2xl mb-4 animate-pulse`}
//                       >
//                         {section.icon}
//                       </div>
//                       <h3 className="text-2xl font-serif font-bold text-gray-800 mb-2">{section.title}</h3>
//                     </div>

//                     {/* Preview of first 2 items */}
//                     <div className="space-y-3">
//                       {section.items.slice(0, 2).map((item, itemIndex) => (
//                         <div
//                           key={itemIndex}
//                           className="flex items-start space-x-3 group-hover:translate-x-1 transition-transform duration-300"
//                           style={{ transitionDelay: `${itemIndex * 50}ms` }}
//                         >
//                           <div
//                             className={`w-2 h-2 rounded-full bg-gradient-to-r ${section.gradient} mt-2 flex-shrink-0`}
//                           />
//                           <p className="text-gray-700 text-sm leading-relaxed">{item}</p>
//                         </div>
//                       ))}
//                       {section.items.length > 2 && (
//                         <div className="text-center pt-4">
//                           <span className="text-gray-600 text-sm">Click to see all recommendations →</span>
//                         </div>
//                       )}
//                     </div>

//                     {/* Hover Glow Effect */}
//                     <div
//                       className={`absolute inset-0 rounded-3xl bg-gradient-to-r ${section.gradient} opacity-0 group-hover:opacity-20 transition-opacity duration-500 pointer-events-none`}
//                     />
//                   </button>
//                 </div>
//               </ScrollReveal>
//             ))}
//           </div>
//         </div>

//         {/* Daily Practice Section */}
//         <ScrollReveal>
//           <div className="max-w-4xl mx-auto px-4 pb-20">
//             <div className="glass-card rounded-3xl p-8 bg-pink-100/50">
//               <div className="text-center mb-8">
//                 <GradientText className="text-3xl font-serif font-bold mb-4">Daily Practice Reminder</GradientText>
//                 <p className="text-gray-700">Consistency is key to balancing your {currentDosha.name} constitution</p>
//               </div>

//               <div className="grid md:grid-cols-3 gap-6">
//                 <div className="text-center">
//                   <div className="w-12 h-12 rounded-full bg-gradient-to-r from-pink-400 to-rose-500 flex items-center justify-center text-xl mb-3 mx-auto animate-spin-slow">
//                     🧘
//                   </div>
//                   <h4 className="font-semibold text-gray-800 mb-2">Morning Practice</h4>
//                   <p className="text-sm text-gray-600">Start with meditation and pranayama</p>
//                 </div>

//                 <div className="text-center">
//                   <div className="w-12 h-12 rounded-full bg-gradient-to-r from-rose-400 to-pink-500 flex items-center justify-center text-xl mb-3 mx-auto animate-bounce-slow">
//                     🍃
//                   </div>
//                   <h4 className="font-semibold text-gray-800 mb-2">Mindful Eating</h4>
//                   <p className="text-sm text-gray-600">Follow your dosha-specific diet</p>
//                 </div>

//                 <div className="text-center">
//                   <div className="w-12 h-12 rounded-full bg-gradient-to-r from-pink-500 to-rose-600 flex items-center justify-center text-xl mb-3 mx-auto animate-pulse">
//                     🌙
//                   </div>
//                   <h4 className="font-semibold text-gray-800 mb-2">Evening Routine</h4>
//                   <p className="text-sm text-gray-600">Wind down with gentle yoga</p>
//                 </div>
//               </div>
//             </div>
//           </div>
//         </ScrollReveal>
//       </div>

//       {/* Content Modal */}
//       <ContentModal
//         isOpen={modalContent.isOpen}
//         onClose={closeModal}
//         title={modalContent.title}
//         items={modalContent.items}
//         icon={modalContent.icon}
//         gradient={modalContent.gradient}
//       >
//         {modalContent.title === "Take a Quick Quiz" && quizStep < quizQuestions.length && (
//           <div className="space-y-4">
//             <h3 className="text-xl font-bold text-gray-800">{quizQuestions[quizStep].question}</h3>
//             {quizQuestions[quizStep].options.map((option, idx) => (
//               <button
//                 key={idx}
//                 onClick={() => handleAnswer(option)}
//                 className="w-full py-2 px-4 rounded-lg bg-gradient-to-r from-pink-300 to-rose-400 text-white text-sm font-medium hover:bg-opacity-90 transition"
//               >
//                 {option}
//               </button>
//             ))}
//           </div>
//         )}
//         {modalContent.title === "Take a Quick Quiz" && quizStep >= quizQuestions.length && feedback && (
//           <div className="space-y-4 text-center">
//             <h3 className="text-xl font-bold text-gray-800">Your Feedback</h3>
//             <p className="text-gray-700">{feedback}</p>
//             <button
//               onClick={closeModal}
//               className="w-full py-2 px-4 rounded-lg bg-gradient-to-r from-pink-300 to-rose-400 text-white text-sm font-medium hover:bg-opacity-90 transition"
//             >
//               Close
//             </button>
//           </div>
//         )}
//       </ContentModal>

//       <YogaPosesModal isOpen={isYogaPosesOpen} onClose={() => setIsYogaPosesOpen(false)} />

//       <div className="absolute top-32 left-10 opacity-50">
//         <Lotus3D />
//       </div>
//       <div className="absolute top-40 right-20 opacity-60">
//         <Chakra3D size={80} color="pink" speed={0.5} />
//       </div>
//       <div className="absolute bottom-40 left-1/4 opacity-40">
//         <Chakra3D size={60} color="rose" speed={1.2} />
//       </div>

//       <style jsx>{`
//         @keyframes spin-slow {
//           from { transform: rotate(0deg); }
//           to { transform: rotate(360deg); }
//         }

//         @keyframes bounce-slow {
//           0%, 100% { transform: translateY(0); }
//           50% { transform: translateY(-10px); }
//         }

//         .animate-spin-slow {
//           animation: spin-slow 8s linear infinite;
//         }

//         .animate-bounce-slow {
//           animation: bounce-slow 3s ease-in-out infinite;
//         }

//         .glass-card {
//           background: rgba(255, 255, 255, 0.5);
//           backdrop-filter: blur(10px);
//           border: 1px solid rgba(255, 192, 203, 0.3);
//         }
//       `}</style>

//       {/* Chatbot fixed at bottom-right */}
//       <Chatbot />
//       <Footer />
//     </div>
//   )
// }